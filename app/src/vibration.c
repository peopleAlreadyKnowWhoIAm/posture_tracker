...
static const struct gpio_dt_spec vibration_control = GPIO_DT_SPEC_GET(DT_NODELABEL(vibration_output), gpios);
...
void vibration_short_start(void) {
    ...
    is_vibrating = true;
   gpio_pin_set_dt(&vibration_control, 1);
...
static int vibration_gpio_init(void) {
    ...
    int ret = gpio_pin_configure_dt(&vibration_control, GPIO_OUTPUT_INACTIVE);
    return ret;
}
